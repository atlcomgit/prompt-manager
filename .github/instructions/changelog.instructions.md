...
- Автор: 🅰️🅻🅴🅺.
- Ветка: master.
- Что сделано: автоматическое создание названия теперь корректно завершается в актуальном окне промпта после
  его повторного открытия или переименования.
- Ключевые моменты: завершающее состояние отправляется независимо от полного обновления, а отложенное открытие
  сверяется с текущим состоянием фоновой генерации.
- Файлы:
    CHANGELOG.md
    README.md
    src/providers/editorPanelManager.ts
    src/utils/promptSaveFeedback.ts
    tests/editorPanelManager.test.ts
    tests/promptSaveFeedback.test.ts

## Без номера: Защищённая схема project.instructions.md

- Дата: 2026-09-04.
- Автор: 🅰️🅻🅴🅺.
- Ветка: master.
- Что сделано: Общая инструкция теперь разрешает обновлять только отдельную секцию актуальной схемы проекта,
  а все пользовательские разделы сохраняет без изменений и не ограничивает их размер.
- Ключевые моменты: Управляемая секция ограничена уникальными маркерами, 300 строками и 30 000 кодовых точек
  Unicode; первое создание и последующие обновления выполняются условными атомарными патчами; повреждённые
  границы, параллельные изменения и прямой запрет пользователя останавливают запись; журнал изменений больше
  не загружается целиком как постоянный контекст; записи независимых задач могут добавляться параллельно и
  должны сохраняться без перезаписи, а конфликтом считается только несовместимое изменение одной записи.
- Файлы:
  CHANGELOG.md
  share/general.instructions.md
...
- Автор: 🅰️🅻🅴🅺.
- Ветка: master.
- Что сделано: В индивидуальных списках виджета `Ветки проектов` существующая ветка промпта теперь получает понятную подпись `промпт, есть`, а отсутствующая ветка сохраняет прежнюю подпись `промпт`.
- Ключевые моменты: Наличие ветки определяется только по проектному признаку `exists`, доступность переключения и создание отсутствующей ветки не изменяются, общий список `Для всех` сохраняет формат `ветка (N/M)`, а проверки покрывают существующую, отсутствующую, недоступную и одновременно текущую ветку промпта.
- Файлы:
  CHANGELOG.md
  README.md
  src/webview/editor/components/PromptDashboard.tsx
...
## 162: Добавить колонку и экспорт Excel

- Дата: 2026-07-26.
- Автор: 🅰️🅻🅴🅺.
- Ветка: master.
- Что сделано: В последней таблице статистики добавлено отдельное время «На разное», текущую отфильтрованную и отсортированную таблицу теперь можно сохранить в настоящий файл Excel, а все восемь колонок умещаются по ширине страницы благодаря компактным отступам и сокращению длинных названий.
- Ключевые моменты: Неучтённое время берётся из `timeSpentUntracked` или дневной записи выбранного периода и продолжает входить в общий итог; XLSX получает восемь колонок и итоговую строку в текущем порядке строк; экранная таблица использует фиксированные доли колонок суммарно на 100%, оставляет 30% ширины названию и показывает его полный текст в подсказке; текст Excel сохраняется без выполнения формул; создание книги выполняется только после выбора файла; большие входные сообщения ограничиваются допустимыми размерами; существующие HTML/Markdown-экспорты не изменены.
- Файлы:
  CHANGELOG.md
  README.md
  package-lock.json
  ...
  src/types/messages.ts
  src/types/prompt.ts
  src/utils/statisticsTable.ts
  src/utils/statisticsXlsxExport.ts
  src/webview/statistics/StatisticsApp.tsx
  tests/statisticsPanelManager.test.ts
  tests/statisticsXlsxExport.test.ts
...
- Автор: 🅰️🅻🅴🅺.
- Ветка: master.
- Что сделано: В таблице контейнеров действия перенесены в компактное меню, исправлено надежное открытие этого меню и добавлена заметная реакция пунктов на наведение и нажатие; все карточки теперь сворачиваются до строки состояния, а показатели CPU, RAM и сети в списке показываются тремя небольшими графиками только после раскрытия строки.
- Ключевые моменты: Меню закрывается по фактическому уходу фокуса или внешнему нажатию и не схлопывается при переводе фокуса на свои пункты; интерактивные состояния используют системные цвета VS Code и не применяются к недоступным командам; существующие команды и их отдельные индикаторы выполнения сохранены через общие описания действий; состояния карточек используют прежнее хранилище раскрытых элементов; обновление показателей работает только для видимых раскрытых блоков.
- Файлы:
  CHANGELOG.md
  README.md
  src/webview/editor/components/PromptDashboard.tsx
