...
### Added
- Prompt editor overview dashboard: when the editor has enough right-side space, flat compact masonry-style widgets now sit directly in the empty area beside the fixed-width prompt form instead of using a separate drawer. Widgets load independently through a stale-while-revalidate cache warmed every five minutes and cover recent long-running prompts with task numbers, current prompt status with a larger color-coded chip plus an In Progress-only prompt-list-style progress bar, a dedicated project-branches widget for bulk branch switching, separate project commits, MR/PR, and `Параллельные ветки` widgets, explorer-style file views, standard side-by-side VS Code diff opening for the exact commit or branch change, and automatic cached AI review when dashboard inputs change.
- Dashboard refinements: active prompt rows now open the selected prompt, project branch switching is denser and preset buttons apply per-project prompt/tracked branches immediately, project selectors prefer the prompt branch when it exists, dashboard project scope falls back to all workspace repos when saved selections are stale, dashboard widgets mirror the prompt form left-accent card treatment, parallel-branch summaries now keep structured file diffs with rename support and show compressed explorer-like folder trees from the same merge-base range used by diff opening, file-open actions show an inline loader, remote GitHub/GitLab CLI failures are shown as compact availability messages, and AI review reuses shared cached results across prompt switches until the dashboard fingerprint changes.
...
### Fixed
- Prompt editor dashboard no longer falls back to `Анализ обновляется...` on prompt switches when the prompt/Git fingerprint is unchanged; the initial snapshot now reuses shared project and AI-review cache data immediately, project pipelines resolve against the latest workflow or pipeline run on each repository's actual current branch, branch actions keep their request-scoped loaders visible until the final refreshed snapshot lands, the `Активные промпты` widget falls back from an empty yesterday bucket to the latest earlier active day, `Ветки проектов` keeps the chosen target branch visible until refreshed data lands, and commit/parallel change files render as a real nested tree instead of grouped folders.
- Prompt switching is no longer slowed down by hidden or immediately reloading dashboard widgets: the editor skips dashboard requests when the right-side rail is not visible, automatic widget refresh is delayed and coalesced away from the prompt-switch critical path, stale in-flight dashboard work is paused as soon as prompt switching starts, and Prompt Manager debug logs now split prompt-open pending-save/storage timings plus dashboard performance timings for diagnosis.
