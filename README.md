...
- Reorder prompt-dashboard cards by dragging the handle in each section header; the order is shared across prompts in the workspace, persists across prompt switches and editor reloads, shows a visible insertion indicator while you drag, and now keeps the exact visible column placement instead of rebalancing cards to another spot after drop.
