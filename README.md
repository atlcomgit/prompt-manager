...
- Keep prompt saves resilient during fast switching: save-time Description enrichment is now disabled entirely so the field changes only through the manual `Сгенерировать через ИИ` action, prompt reopen can reuse a pending saved snapshot by `promptUuid` during rename or switch flows, newer local edits are preserved when they land while an older save is still in flight, and self-authored `plan.md` / `report.txt` writes no longer bounce back through the external-config watcher path.
...
- Контролируйте локальные Docker Compose контейнеры прямо из prompt dashboard: виджет `Docker контейнеры` находит root-level compose-файлы проектов рабочей области с учётом `Excluded Paths`, на самом первом рендере переиспользует persisted workspace cache только когда текущий hash compose-файлов совпадает, сразу после этого запускает отдельную background-перепроверку live-состояния Docker, показывает CPU, RAM и сеть только для запущенных контейнеров, причём RAM теперь выводится в МБ вместо процентов, а после 1000 МБ автоматически переключается на ГБ с двумя знаками после запятой, в режиме `Карточки` compose-карточки тоже показывают компактные суммарные CPU/RAM числа для запущенных сервисов, а под блоком ресурсов контейнера рисуется лёгкий 5-минутный history chart по CPU/RAM/сети с адаптивным масштабом и downsample до ограниченного числа точек, растягивает сам SVG-график на всю доступную ширину, рисует сеть поверх остальных серий фиксированной ярко-оранжевой линией, использует стабильные абсолютные time buckets для downsample, а после длинной паузы скрытого виджета начинает новую историю без перетасовки старых точек и диагональных артефактов; live polling держится только пока соответствующий Docker resource block реально показан в раскрытой и видимой секции dashboard, а внешние Docker-изменения при свернутом виджете откладываются до следующего раскрытия, чтобы не увеличивать частоту Docker polling и не нагружать VS Code лишним SVG-рендером, по умолчанию обновляет live-метрики лёгким stats-only background refresh раз в секунду, включая CPU/RAM-колонки в режиме `Таблица`, запоминает выбранный вид и раскрытые элементы, группирует карточки и список по проектам, в режиме `Список` держит контейнеры и declared services под их compose-файлом, а таблицу показывает плоским списком `проект/compose/контейнер`; в дереве раскрывает branch-guided `проект -> compose-файл -> контейнеры/services`, показывает статус compose-файла и объявленные сервисы во всех режимах даже когда контейнеров нет, даёт открыть сам compose-файл из виджета, выполняет compose-file `up` / `down` / `restart` скрыто с loader на конкретной кнопке до прихода фактического widget refresh, повторно дочитывает состояние после start/stop/restart/up/down перед разблокировкой кнопок, корректно применяет status filter и сортировку, открывает логи контейнера в отдельном терминале, показывает play для незапущенного compose-файла и restart для запущенного, отображает статус контейнера в таблице и дереве компактной квадратной outlined-иконкой, держит disclosure контейнера выровненным по строке его названия, показывает ошибку compose-команды рядом с compose-файлом, а действия контейнера показывает theme-aware прозрачными icon-only кнопками справа в строке контейнера с loader на конкретной кнопке, `play` для остановленных контейнеров и restart для запущенных.
- В режиме `Таблица` Docker widget теперь переносит disclosure `Показать детали` в первую колонку, раскрывает details и по клику на имя контейнера, убирает лишнюю compose-иконку перед названием контейнера, оставляет в table title только однострочное имя контейнера с ellipsis для длинных названий, переносит составную строку `проект/compose/контейнер` в низ раскрытого details-блока как `Контейнер: ...`, делает запущенные строки визуально ярче остановленных, показывает внутри раскрытых table-details тот же live resource block с CPU/RAM/сетью и history chart, исправляет search так, чтобы скрытые inactive service/container rows не оставались видимыми по старому и чтобы поиск находил non-running контейнеры даже когда host compose-group держит только активные rows, добавляет clear-кнопку в поле поиска и запоминает введённый search, выбранный filter и sort между перезапусками и переключениями prompt dashboard, а в summary tiles `Запущено` и `Остановлено` показывает icon-only batch actions справа от числового счётчика: если в рабочей области уже есть running контейнеры, блок `Запущено` переключается на общий restart этих контейнеров; если running нет, там остаётся запуск сохранённого предыдущего набора; остановка workspace-контейнеров теперь сперва делает `docker compose down` для compose-managed групп и только потом останавливает standalone контейнеры в обратном порядке, запуск предыдущего набора восстанавливает сохранённые compose-сервисы и standalone контейнеры после перезапуска VS Code, а внешние `docker start/stop/restart` команды теперь быстрее отражаются в виджете через long-lived Docker event stream с автоматическим переподключением после close/error.
- Docker CPU polling now requests single snapshots without Docker `one-shot`, so Linux daemons keep `precpu` deltas and live CPU percentages in the widget no longer get stuck at `0%`.
- Docker CPU in the widget is now normalized to the host-wide `0..100%` scale instead of Docker CLI's multi-core sum, so one busy container no longer renders values above `100%` in the dashboard.
...
- Pending manual branch choices in `Ветки проектов` now survive widget refreshes and switching between selected rows and `Показать все` until the refreshed Git data confirms the new current branch.
...
- Refresh any dashboard card directly from its own header: `Статус промпта`, `Активные промпты`, `AI review`, and the Git-backed cards now expose a small refresh button beside the section counter, while the shared Git cards (`Ветки проектов`, `Коммиты проектов`, `Параллельные ветки`, `MR/PR`) reuse the same targeted `projects` refresh path instead of forcing a full rail reload, keep the loading spinner only on the card you actually refreshed, and show the same local spinner when an empty or stale card auto-refreshes right after expand.
- When `Ветки проектов` temporarily widens to all workspace repositories, the shared `projects` refresh now reuses already loaded prompt-project rows and fetches new Git data only for the still-missing repositories, so broad branch refreshes land faster on large workspaces.
- Collapse prompt-dashboard widgets from the same section header and keep that state shared for the whole workspace instead of per prompt; collapsed cards hide their body and refresh button, the host skips snapshot/widget refresh and lazy hydration for those sections, the shared Git `projects` payload is no longer refreshed once all project-backed cards are collapsed, the very first editor paint now respects that persisted collapsed state instead of flashing every card open first, reopening a collapsed card now immediately starts a full or targeted refresh again when the visible widget is still empty or stale, and reopening the first Git-backed card from a fully collapsed prompt page now bootstraps from that single card's widget payload instead of forcing a full shared `projects` refresh for the other still-collapsed Git cards.
- Reorder prompt-dashboard cards by dragging the handle in each section header; the order is shared across prompts in the workspace, persists across prompt switches and editor reloads, shows a visible insertion indicator while you drag, keeps the exact visible column placement instead of rebalancing cards to another spot after drop, and now also accepts drops in the empty gap between cards or below the last card in a column so releasing slightly off the widget body still commits the nearest visible placement.
- The shared Git-backed cards (`Ветки проектов`, `Коммиты проектов`, `Параллельные ветки`, `MR/PR`) also no longer treat reused warm `projects` cache as if it had already been refreshed during the current collapsed open cycle, so the first expand now starts the expected refresh even when those cards were opened from a hidden state.
- Those shared Git-backed cards now also remember which exact section has already been loaded inside the shared `projects` payload, so one refreshed card no longer renders the other Git cards as if they had already fetched their own data.
- When the first Git-backed card is reopened from a fully collapsed dashboard, its targeted widget refresh now stays authoritative for that expand action instead of being overwritten by an immediate automatic full snapshot request.
- When `promptManager.debugLogging.enabled` is enabled, prompt-dashboard tracing now includes the initial collapse bootstrap plus expand and refresh request decisions from both the editor webview and the host, and those dashboard traces now also pass the Prompt Manager output filter so first-open widget refresh problems can be diagnosed directly from the captured logs.
...
- After `Получить` and `Переключить`, the background AI follow-up now reuses the just-refreshed branch rows instead of starting a second targeted project snapshot first, so those actions settle faster before the review finishes.
...
- Настраивайте Docker-виджет через `promptManager.docker.enabled`, `promptManager.docker.composeFilePatterns`, `promptManager.docker.refreshIntervalMs`, `promptManager.docker.statsIntervalMs`, `promptManager.docker.cacheTtlMs`, `promptManager.docker.defaultViewMode`, `promptManager.docker.terminalShell` и `promptManager.docker.confirmRemove`, если нужно изменить discovery compose-файлов, частоту обновления, срок свежести widget cache, shell для `docker exec` или подтверждение удаления.
