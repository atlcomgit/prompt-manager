...
- **Git Flow commit messages** explicitly activate Kilo Code, apply one 75-second deadline to activation, Git discovery, and its real Source Control command, wait briefly for asynchronously opened nested repositories, read the generated SCM value after completion, and keep the latest request valid across snapshot refreshes. Git Flow явно активирует Kilo Code, применяет общий лимит 75 секунд к запуску, поиску Git-репозитория и штатной Source Control команде, ожидает асинхронное открытие вложенного репозитория, получает текст из SCM после завершения и сохраняет актуальный запрос при обновлении snapshot. After installing a rebuilt extension, reload the VS Code window so the extension host loads the new bundle. После установки новой сборки перезагрузите окно VS Code, чтобы extension host загрузил новый bundle.
