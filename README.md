...
- **Tracker and Statistics** show actual tracked hours when `Hours` is blank for a selected period, include that period in HTML/Markdown previews and exports, keep long metadata values inside responsive HTML cards, and refresh active-prompt time after saving. Статистика показывает фактические часы при пустом поле `Часы`, выводит выбранный период в превью и отчётах, сохраняет длинные значения внутри адаптивных карточек HTML и обновляет время активных промптов после сохранения.
