...
export function buildChatMemoryInstruction(input: ChatMemoryInstructionBuildInput): string {
	const locale = input.locale || 'en';
	const isRussianLocale = locale.toLowerCase().startsWith('ru');
	const usageRules = getInstructionUsageRules(locale);
	const text = isRussianLocale
		? {
			...
			nav1: 'Сначала идут архитектурный контекст и карта проекта.',
			nav2: 'После этого идёт снимок текущих незакомиченных изменений по выбранным проектам.',
			nav3: 'Затем идут недавние и релевантные закомиченные изменения.',
		...
		: {
			...
			nav1: 'Architecture-level context and the project map appear first.',
			nav2: 'A snapshot of current uncommitted changes for the selected projects appears after that.',
			nav3: 'Recent relevant committed changes appear after that.',
