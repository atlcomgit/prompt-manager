...
// Стили оверлея прогресса анализа — плоский дизайн, без тяжёлых теней.
const styles: Record<string, CSSProperties> = {
	...
	// Основной контейнер — плоский, тонкая рамка.
	overlay: {
	...
	summarySection: {
		padding: '16px 22px',
		borderBottom: '1px solid color-mix(in srgb, var(--vscode-foreground) 30%, transparent)',
		display: 'flex',
		flexDirection: 'column',
		gap: '14px',
	},
	...
	// Прогресс-бар — тонкий (8px), плоская заливка.
	progressTrack: {
		height: '8px',
		borderRadius: '999px',
	...
	// Карточка метрики — плоская, с верхним акцентным бордером.
	metricCard: {
		padding: '14px',
		borderRadius: '10px',
	...
	footerSection: {
		...
		borderTop: '1px solid color-mix(in srgb, var(--vscode-foreground) 6%, transparent)',
		display: 'flex',
		flexDirection: 'column',
		gap: '10px',
	...
	tableHead: {
		...
		textAlign: 'left',
		padding: '8px',
		borderBottom: '1px solid color-mix(in srgb, var(--vscode-foreground) 10%, transparent)',
		fontWeight: 700,
		whiteSpace: 'nowrap',
		fontSize: '10px',
		textTransform: 'uppercase',
	...
	// Элемент лога — плоский, без рамки.
	logItem: {
		display: 'flex',
		gap: '10px',
		fontSize: '12px',
